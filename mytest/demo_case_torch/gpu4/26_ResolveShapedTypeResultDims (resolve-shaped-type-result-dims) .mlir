...
func.func private @forward(%arg0: !torch.vtensor<[4],f32>) -> !torch.vtensor<[3],f32> attributes {torch.assume_strict_symbolic_shapes} {
  ...
  %11 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%collapsed : tensor<3xf32>) outs(%10 : tensor<3xf32>) {
  ^bb0(%in: f32, %out: f32):
    %15 = math.absf %in : f32
    linalg.yield %15 : f32
  } -> tensor<3xf32>
  %12 = tensor.empty() : tensor<3xf32>
  %13 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%9, %11 : tensor<3xf32>, tensor<3xf32>) outs(%12 : tensor<3xf32>) {
  ^bb0(%in: f32, %in_0: f32, %out: f32):
    %15 = arith.addf %in, %in_0 : f32
    linalg.yield %15 : f32
  } -> tensor<3xf32>
  %14 = torch_c.from_builtin_tensor %13 : tensor<3xf32> -> !torch.vtensor<[3],f32>
  return %14 : !torch.vtensor<[3],f32>
}
