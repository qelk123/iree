...
func.func private @forward(%arg0: !torch.vtensor<[4],f32>) -> !torch.vtensor<[3],f32> attributes {torch.assume_strict_symbolic_shapes} {
  ...
  %14 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%10, %12 : tensor<3xf32>, tensor<3xf32>) outs(%13 : tensor<3xf32>) {
  ^bb0(%in: f32, %in_14: f32, %out: f32):
    %16 = arith.sitofp %1 : i64 to f32
    %17 = arith.mulf %in_14, %16 : f32
    %18 = arith.addf %in, %17 : f32
